...
export function EventBuilder({ events, onEventCreated }: EventBuilderProps) {
  ...
  // Auto-suggest scoringMode from location unless the admin has manually overridden it
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-1">
        <label htmlFor="eb-name" className="text-sm font-medium text-gray-700">
        ...
        <div className="flex gap-6">
          {(['onstage', 'offstage'] as EventDoc['location'][]).map((loc) => (
            <label key={loc} className="flex items-center gap-2 cursor-pointer select-none">
              ...
              />
              <span className="text-base capitalize">{loc}</span>
            </label>
          ))}
        </div>
      </fieldset>
          ...
          {(['averaged', 'singleByGroup'] as EventDoc['scoringMode'][]).map((mode) => (
            <label key={mode} className="flex items-center gap-2 cursor-pointer select-none">
              <input
                ...
                className="w-5 h-5 accent-blue-600"
              />
              <span className="text-base">{mode === 'averaged' ? 'Averaged' : 'Single by group'}</span>
            </label>
          ))}
        </div>
      ...
      <button
        type="submit"
        disabled={loading}
        className="min-h-[48px] px-6 bg-blue-600 text-white text-base font-medium rounded-lg hover:bg-blue-700 active:bg-blue-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors self-start"
      >
